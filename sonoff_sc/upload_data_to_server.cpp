...
void uploadSensorDataToServer(void) {
  ...
  if (network_status_ok && couter == upload_freq) {
    ...
    sendData();
    couter = 0;
  } else if (network_status_ok &&
             (abs(sensor_dev[3].temp_humi_average[0] -
                  last_temp_humi_average[0]) >= tempthreshold ||
              abs(sensor_dev[3].temp_humi_average[1] -
                  last_temp_humi_average[1]) >= humithreshold ||
